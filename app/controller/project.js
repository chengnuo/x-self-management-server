...
class HomeController extends Controller {
  ...
  async show() {
    const ctx = this.ctx;
    const resDataFindByPk = await ctx.model.Project.findByPk(ctx.params.id);
    ctx.body = {
  ...
  async create() {
    ...
    const resName = await ctx.model.Project.findOne({
      where: {
        name,
      },
    });
    if (resName && resName.id) {
      ...
      const resData = await ctx.model.Project.create({
        name,
        description,
        url,
      });
  ...
  async update() {
    ...
    const resDataFindByPk = await ctx.model.Project.findByPk(id);
    if (!resDataFindByPk) {
      ctx.status = 404;
      return;
    }
