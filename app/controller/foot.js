...
class HomeController extends Controller {
  ...
  async signIn() {
    ...
    const resData = await ctx.model.Foot.findOne(query);
  ...
  async index() {
    ...
    const resData = await ctx.model.Foot.findAndCountAll(query);
    ctx.body = {
      data: resData.rows,
      status: 200,
      message: '获取列表成功',
  ...
  async show() {
    const ctx = this.ctx;
    const resDataFindByPk = await ctx.model.Foot.findByPk(ctx.params.id);
    ctx.body = {
  ...
  async create() {
    ...
    const resName = await ctx.model.Foot.findOne({
      where: {
        name,
      },
    });
    if (resName && resName.id) {
      ...
      const resData = await ctx.model.Foot.create({
        name,
        description,
        url,
      });
  ...
  async update() {
    ...
    const resDataFindByPk = await ctx.model.Foot.findByPk(id);
    if (!resDataFindByPk) {
      ctx.status = 404;
      return;
    }
